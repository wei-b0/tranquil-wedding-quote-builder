...
export default async function DashboardPage({
  ...
  return (
    ...
    >
      ...
      {params.error ? (
        <ToastOnMount message={params.error} type="error" />
      ) : null}
      ...
      {!profileComplete ? (
        <section className="mb-6 rounded-[2rem] border border-[#d7c796] bg-[#fff8e8] p-5 shadow-[0_18px_45px_rgba(48,32,20,0.06)]">
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div>
              <p className="text-xs tracking-[0.3em] text-[#8d7441] uppercase">
                Creator contact required
              </p>
              <h2 className="mt-2 font-serif text-3xl text-stone-900">
                Finish your sales profile before creating quotes
              </h2>
              <p className="mt-2 max-w-2xl text-sm leading-7 text-stone-600">
                Reserve-now and WhatsApp CTAs now use the quote creator&apos;s
        ...
        {quotes.length ? (
          ...
          <div className="rounded-[2rem] border border-dashed border-stone-300 bg-white/70 p-12 text-center">
            ...
            <p className="mt-3 text-sm leading-7 text-stone-600">
              Create your first structured quotation and stop editing PDF files
              manually.
            </p>
          </div>
        )}
